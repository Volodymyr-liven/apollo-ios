...
public protocol WebSocketAuthSource: AnyObject {
    func webSocketTransportWillConnect(_ transport: WebSocketAuthDelegate)
}

public protocol WebSocketAuthDelegate: AnyObject {
    func webSocketTransportReceiveAuthorization(_ accessToken: String)
    func webSocketTransportFail(_ error: (any Error))
}

extension WebSocketTransport: WebSocketAuthDelegate {
    public func webSocketTransportFail(_ error: (any Error)) {
        notifyErrorAllHandlers(error)
    }

    public func webSocketTransportReceiveAuthorization(_ accessToken: String) {
        websocket.request.addValue("Bearer \(accessToken ?? "")", forHTTPHeaderField: "Authorization")
        websocket.connect()
    }
}
...
open class WebSocketTransport {
    ...
    public var authorizationAccessTokenDelegate: WebSocketAuthDelegate? {
        get { config.authorizationAccessTokenDelegate }
        set {
            config.authorizationAccessTokenDelegate = newValue
        }
    }

    public var authSource: WebSocketAuthSource? {
        get { config.authSource }
        set {
            config.authSource = newValue
        }
    }
    ...
    public struct Configuration {
        ...
        public fileprivate(set) var authorizationAccessTokenDelegate: WebSocketAuthDelegate?
        public fileprivate(set) var authSource: WebSocketAuthSource?
        ...
        public init(
            ...
            authorizationAccessTokenDelegate: WebSocketAuthDelegate? = nil,
            authSource: WebSocketAuthSource? = nil
        ) {
            ...
            self.authorizationAccessTokenDelegate = authorizationAccessTokenDelegate
    ...
    public init(
        ...
        self.addApolloClientHeaders(to: &self.websocket.request)
        self.config.authorizationAccessTokenDelegate = self
        ...
        if config.connectOnInit {
            if self.authSource == nil {
                ...
                self.authSource?.webSocketTransportWillConnect(self)
            }
        }
